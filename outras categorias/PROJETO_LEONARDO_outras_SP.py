...
LATITUDE_PESQUISA = "-23.456906879013165"
LONGITUDE_PESQUISA = "-46.33272887281552"
...
def fetch_ids(category_alias, LATITUDE_PESQUISA, LONGITUDE_PESQUISA):
    try:
        ...
        for card_idx_loop, card in enumerate(response.json()['sections'][section_idx]['cards']):
            if card["cardType"] == "MERCHANT_LIST_V2":
                card_idx = card_idx_loop
                break
...
def main():
    category_aliases = ["MERCADO_BEBIDAS", "HOME_MERCADO_BR", "MERCADO_FARMACIA",
                        "MERCADO_PETSHOP", "SHOPPING_OFICIAL"]
